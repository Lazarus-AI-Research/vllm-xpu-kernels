...
// (kernels/quantization/nvfp4_gemv/variants/xpu_sycl/nvfp4_gemv.sycl.cpp) so
// the kernel is editable in-tree. This is the entire numeric core: edit the
// decode helpers or the GEMV loop below and rebuild with ./build.sh.
//
// Format (matches ModelOpt NVFP4, swizzle=False):
//   weight W[N, K]   e2m1 fp4, packed 2 nibbles/byte along K (low nibble = even
//   k) scale  S[N, K/16] one e4m3 (fp8) byte per 16 weight elements global
//   per-tensor fp32 scale (amax / (6*448)) dequant:  w = e2m1(nibble) *
//   e4m3(block_scale) * global_scale
//
...
using WVec =
    sycl::vec<std::uint32_t, 4>;  // 16 bytes = 32 fp4 = two nvfp4 blocks
...
template <typename T>
sycl::event nvfp4_gemv_typed(
    sycl::queue& q,
    const std::uint8_t* w,
    const std::uint8_t* bscale,
    float gscale,
    const T* x,
    T* y,
    std::size_t N,
  ...
  return q.parallel_for<Nvfp4GemvKernel<T>>(
      ndr, [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(kSG)]] {
        ...
        const WVec* wvrow =
            reinterpret_cast<const WVec*>(w + n * bytes_per_row);
...
template <typename T>
sycl::event nvfp4_gemm_mtiled_typed(
    sycl::queue& q,
    const std::uint8_t* w,
    const std::uint8_t* bscale,
    float gscale,
...
inline sycl::event nvfp4_gemm_mtiled_launch(
  ...
  switch (act_dt) {
    case ActDType::f32:
      return nvfp4_gemm_mtiled_typed(
          q,
          w,
          s,
          global_scale,
...
inline sycl::event nvfp4_gemv_launch(
    sycl::queue& q,
    const void* w_packed,
    const void* block_scales,
    float global_scale,
