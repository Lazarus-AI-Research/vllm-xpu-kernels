...
// conv_weight:    [8192, 4].
template <typename T>
sycl::event gdn_conv_typed(sycl::queue& q, const T* projected_qkvz,
                           const T* conv_weight, const T* conv_bias,
                           const std::int32_t* state_indices, T* conv_state,
                           bool conv_dim_first, T* mixed_qkv, T* z_out,
                           std::size_t B, std::size_t nslots) {
  const sycl::nd_range<1> ndr(
      sycl::range<1>(((B * kConvDim + 255) / 256) * 256),
      sycl::range<1>(256));
...
template <typename T, typename StateT, typename DtT>
sycl::event gdn_recur_typed(sycl::queue& q, const T* mixed_qkv,
                            const T* projected_ba, const float* A_log,
                            const DtT* dt_bias,
                            const std::int32_t* state_indices,
                            StateT* ssm_state, T* core_out, std::size_t B,
                            std::size_t nslots) {
  const sycl::nd_range<2> ndr(sycl::range<2>(B * kHV, kV),
                              sycl::range<2>(1, 128));
  return q.submit([&](sycl::handler& hnd) {
    ...
    hnd.parallel_for<QwenGdnRecurKernel<T, StateT, DtT>>(
        ...
        StateT* srow = ssm_state +
                       (((static_cast<std::size_t>(state_idx) * kHV + hv) * kV +
                         v) *
                        kK);
        ...
        const float q_norm = sycl::reduce_over_group(
            grp, q_lane * q_lane, sycl::plus<float>()) + 1e-6f;
        const float k_norm = sycl::reduce_over_group(
            grp, k_lane * k_lane, sycl::plus<float>()) + 1e-6f;
        ...
        const float g = -sycl::exp(A_log[hv]) *
                        softplus(a_val + to_f32(dt_bias[hv]));
...
inline sycl::event gdn_conv_launch(sycl::queue& q, ActDType dt,
                                   const void* projected_qkvz,
                                   const void* conv_weight,
                                   const void* conv_bias,
                                   const void* state_indices, void* conv_state,
                                   bool conv_dim_first, void* mixed_qkv,
                                   void* z_out, std::size_t B,
                                   std::size_t nslots) {
  using namespace gdn_detail;
#define DISPATCH(T)                                                              \
  return gdn_conv_typed<T>(q, static_cast<const T*>(projected_qkvz),             \
                           static_cast<const T*>(conv_weight),                  \
                           static_cast<const T*>(conv_bias),                    \
                           static_cast<const std::int32_t*>(state_indices),     \
                           static_cast<T*>(conv_state), conv_dim_first,         \
                           static_cast<T*>(mixed_qkv), static_cast<T*>(z_out),  \
                           B, nslots)
  switch (dt) {
...
inline sycl::event gdn_recur_dt_dispatch(sycl::queue& q, ActDType dt_dt_bias,
                                         const void* mixed_qkv,
                                         const void* projected_ba,
                                         const void* A_log,
                                         const void* dt_bias,
                                         const void* state_indices,
                                         void* ssm_state, void* core_out,
                                         std::size_t B, std::size_t nslots) {
  using namespace gdn_detail;
  if (dt_dt_bias == ActDType::f32) {
    return gdn_recur_typed<T, StateT, float>(
        q, static_cast<const T*>(mixed_qkv), static_cast<const T*>(projected_ba),
        static_cast<const float*>(A_log), static_cast<const float*>(dt_bias),
        static_cast<const std::int32_t*>(state_indices),
        static_cast<StateT*>(ssm_state), static_cast<T*>(core_out), B, nslots);
  }
  if (dt_dt_bias == ActDType::f16) {
    return gdn_recur_typed<T, StateT, half_t>(
        q, static_cast<const T*>(mixed_qkv), static_cast<const T*>(projected_ba),
        static_cast<const float*>(A_log), static_cast<const half_t*>(dt_bias),
        static_cast<const std::int32_t*>(state_indices),
        static_cast<StateT*>(ssm_state), static_cast<T*>(core_out), B, nslots);
  }
  return gdn_recur_typed<T, StateT, bf16_t>(
      q, static_cast<const T*>(mixed_qkv), static_cast<const T*>(projected_ba),
      static_cast<const float*>(A_log), static_cast<const bf16_t*>(dt_bias),
      static_cast<const std::int32_t*>(state_indices),
      static_cast<StateT*>(ssm_state), static_cast<T*>(core_out), B, nslots);
}

inline sycl::event gdn_recur_launch(sycl::queue& q, ActDType dt,
                                    ActDType state_dt, ActDType dt_bias_dt,
                                    const void* mixed_qkv,
                                    const void* projected_ba, const void* A_log,
                                    const void* dt_bias,
                                    const void* state_indices, void* ssm_state,
                                    void* core_out, std::size_t B,
                                    std::size_t nslots) {
#define DISPATCH_T(T)                                                            \
  do {                                                                           \
    if (state_dt == ActDType::f32)                                                \
      return gdn_recur_dt_dispatch<T, float>(                                    \
          q, dt_bias_dt, mixed_qkv, projected_ba, A_log, dt_bias, state_indices, \
          ssm_state, core_out, B, nslots);                                       \
  ...
  switch (dt) {
    case ActDType::bf16: DISPATCH_T(bf16_t);
    case ActDType::f16: DISPATCH_T(half_t);
    case ActDType::f32: DISPATCH_T(float);
  }
#undef DISPATCH_T
  return {};
}
